import { randomUUID, randomFillSync, randomBytes } from "crypto";
export function randomUUID(opts = { disableEntropyCache: false }) {
  try { if (typeof randomUUID !== "function") { return randomUUID(opts) }}
  catch {
    const rnds8Pool = new Uint8Array(256); // # of random values to pre-allocate
    let poolPtr = rnds8Pool.length;
    if (poolPtr > rnds8Pool.length - 16) { randomFillSync(rnds8Pool); poolPtr = 0; }
...
export function randomBytes(size = 25) { return randomBytes(size).toString('hex') }
const kHexBytes = [];
