...
async function facebookdl(url) {
    types_js_1.FacebookDownloaderArgsSchema.parse(arguments);
    ...
    const { data: { id, thumbnail, duration, a, av, v } } = await (0, got_1.default)('https://youtube4kdownloader.com/ajax/getLinks.php', {
        ...
        searchParams: {
            video: url,
            rand: JSON.stringify((0, crypto_1.randomBytes)(13))
        }
    }).json();
    const result = (a || [])
        .concat(av)
    ...
    return types_js_1.FacebookDownloaderSchema.parse(res);
}
...
async function facebookdlv2(url) {
    ...
    const decodeParams = (0, utils_js_1.getEncodedSnapApp)(res);
    let html;
    ...
    return types_js_1.FacebookDownloaderV2Schema.parse(data);
}
...
async function facebookdlv3(url) {
    ...
    return types_js_1.FacebookDownloaderV3Schema.parse(res);
}
